...
func (postgres *PostgresDB) GetUrl(shortUrl string) (string, error) {
	if shortUrl == "" {
		return "", errors.New("DB: wrong initial data to read")
	}
...
func (postgres *PostgresDB) PostUrl(shortUrl string, longUrl string) error {
	if shortUrl == "" || longUrl == "" {
		return errors.New("DB: wrong initial data to write")
	}
	...
	return errors.New("DB: already have this link")
}

func (postgres *PostgresDB) Available(shortUrl string) (bool, error) {
	if shortUrl == "" {
		return false, errors.New("DB: wrong initial data to write")
	}
